...
function chart(P, {bars=false, barStyle="solid", label="ACTIVITY", sub="/ LAST 31 DAYS"}={}){
  ...
  const eyebrow=`<text x="${padL}" y="58" font-size="18" font-weight="800" fill="${P.ink}" font-family="${sans}">Contribution Activity</text><text x="${W-padR-OFF}" y="58" font-size="10" letter-spacing="2" font-weight="700" font-family="${mono}" text-anchor="end"><tspan fill="${P.line}">//</tspan><tspan fill="${P.faint}">&#160;LAST 31 DAYS · ${total} TOTAL · MAX ${max}/DAY</tspan></text>`;
